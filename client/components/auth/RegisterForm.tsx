...
import { RegistrForm } from "@/types/auth.types"; // Fixed typo alias if needed
import { registerSchema } from "@/validations/auth.validate";
import { registerService } from "@/services/auth.service";
import { useAuth } from "@/hooks/useAuth";
import { registerFields } from "@/config/auth.config";
...
export default function RegisterForm({ toggle }: { toggle: () => void}) {
  ...
  const validate = (formData: RegistrForm ) => {
      ...
      router.push("/");
        ...
        switch(err.status){
            case 400:
                ...
                break;
            case 409:
                errorObj.email = "User already exist login instead";
                break;
            case 500:
        ...
        
        {
            registerFields.map((field, index) => (
                <Input
                    ...
                    value={form[field.name as keyof RegistrForm]}
